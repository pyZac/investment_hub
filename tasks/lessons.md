...
## 2026-09-24 — post-phase (Direct Commission rule change: first-purchase-only removed)
Note (not a mistake — a real investigation that changed course based on
what was found, worth recording for the process). Started from a bug
report ("no commission paid for a Platinum/Diamond purchase, only for the
old seeded package") that named a specific suspected mechanism (a
per-package `directCommissionRate` field being 0/null). Investigation
found: (1) `Package` has no such field at all — commission math has
always read only the global `commissionConfig.directRate`, matching
mlm_rules_log.md's design (one admin-editable versioned rate, no
per-package override was ever specified); (2) querying this dev DB for
the described purchases found zero matching investments — the reported
scenario happened on the live production app, not here, and I have no
production DB access; (3) the far more likely explanation, given
"first-ever purchase paid, later ones didn't," was `Direct Commission`'s
existing-by-design `isDirectCommissionTriggerPurchase` first-purchase
-only gate working exactly as specified (mlm_rules_log.md Section 4, and
docs/phases/phase-06-direct-commission.md) — not a bug. Reported this
finding back rather than guessing at a fix for the originally-suspected
(and non-existent) field. The user then made an explicit, deliberate
business-rule change decision on the spot: remove the first-purchase-only
gate entirely so Direct Commission pays on every purchase going forward.
Implemented as instructed: deleted `isDirectCommissionTriggerPurchase`
entirely (its one real call site was the only thing gating on it; two
other files only referenced it in comments, not code — updated those
comments too so they don't dangle-reference a deleted function name),
updated `direct-commission.test.ts` (deleted its now-meaningless describe
block, flipped the "non-first purchase" test to assert the new behavior
with a real amount cross-check, added a two-purchase test proving BOTH
pay commission), and updated `mlm_rules_log.md` Section 4 plus its
Section 6 MRV comment (which explicitly contrasted MRV against Direct
Commission's old first-purchase-only rule — now stale, fixed in the same
pass since it was in the same file being edited). Left historical/planning
docs (`build_plan.md`, `phase-06-direct-commission.md`,
`SESSION_PROMPTS.md`) untouched — user only named `mlm_rules_log.md`, and
phase briefs are historical records of what was built at the time, not
living specs to rewrite after the fact.
Also caught (confirmed with user before touching, since it wasn't in the
original instructions): `messages/en.json`/`ar.json`'s
`commissionHistoryDescription` (the Referrals page's own UI copy)
literally said "when your direct referrals make their first purchase" —
would have actively misled users about their own commission history the
moment this rule shipped. Fixed in both locales after asking, rather than
either silently expanding scope or silently leaving known-wrong
user-facing copy in place.
Rule: (1) when a bug report names a specific suspected mechanism, verify
that mechanism actually exists in the schema/code before investigating
further under that assumption — here the named field (`directCommissionRate`
on `Package`) simply didn't exist, which took one grep to rule out and
avoided wasted effort chasing a fix for something that couldn't be the
cause. (2) When investigation reveals the reported symptom is actually
correct, designed behavior (not a bug), report that finding plainly and
let the project owner decide whether to change the underlying rule —
don't unilaterally decide it's "probably fine" and drop the ticket, and
don't unilaterally start changing business rules without being asked,
even when a rule change seems like the obvious next step. (3) When a
gating function is deleted, grep for every reference to its NAME (not
just its call sites) — comment-only references in unrelated files
(binary-tree.ts, rank.ts) still need updating so they don't describe
behavior/functions that no longer exist, even though removing them
