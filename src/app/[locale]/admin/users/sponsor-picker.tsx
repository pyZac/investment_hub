...
type SponsorOption = { id: string; name: string; email: string };

export function SponsorPicker({
  value,
  onChange,
}: {
  value: SponsorOption | null;
  onChange: (sponsor: SponsorOption | null) => void;
}) {
  const t = useTranslations("AdminUsers");
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<SponsorOption[]>([]);
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      searchUsersAction(query, 1).then((result) => {
        if (result.ok) {
          setResults(result.users.map((u) => ({ id: u.id, name: u.name, email: u.email })));
        }
      });
    }, 300);
  ...
  return (
    <div ref={containerRef} className="relative space-y-1.5">
      <Label htmlFor="sponsor-search">{t("sponsorLabel")}</Label>
      <Input
        id="sponsor-search"
        value={query}
        onChange={(e) => {
      ...
      {open && query.trim().length > 0 && (
        <div className="absolute z-10 mt-1 w-full max-h-56 overflow-y-auto rounded-lg border border-border/60 bg-popover shadow-md shadow-black/20">
          {results.length === 0 ? (
            <p className="px-3 py-2 text-sm text-muted-foreground">{t("sponsorNoResults")}</p>
          ) : (
            results.map((r) => (
              <button
                key={r.id}
