...
import { searchRecipientsAction, type RecipientOption } from "./actions";
...
export function RecipientPicker({
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      searchRecipientsAction(query).then((result) => {
        if (result.ok) {
  ...
  return (
    <div ref={containerRef} className="relative space-y-1.5">
      <Label htmlFor="transfer-recipient-search">{t("recipientLabel")}</Label>
      <Input
      ...
      {open && query.trim().length > 0 && (
        <div className="absolute z-10 mt-1 w-full max-h-56 overflow-y-auto rounded-lg border border-border/60 bg-popover shadow-md shadow-black/20">
          {results.length === 0 ? (
            <p className="px-3 py-2 text-sm text-muted-foreground">{t("recipientNoResults")}</p>
          ) : (
            results.map((r) => (
              <button
                key={r.id}
